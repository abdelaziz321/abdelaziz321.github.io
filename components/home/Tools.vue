<template>
  <section class="tools mb-40">
    ...
    <div class="row pt-40 pb-20 text-center">
      <div class="col-12">
        <p class="mb-20 subtitle">Usually I use these tools while working, but also I have passion to learn and use new Technologies</p>

        <div class="tools-list text-center">
          <div class="tool" :key="tool.key" v-for="(tool) in tools">
            <component :is="tool.icon" :class="tool.key" />
            <span class="title">
              <span :class="`level mr-3 ${tool.level}`"></span>
              {{ tool.title }}
            </span>
          </div>
        </div>
    ...
    <div class="summary text-left pb-20 pt-25">
      <p>
        <span class="level high mr-15"></span>
        using it regulary
      </p>
      <p>
...
import flask from '@/assets/images/tools/flask.svg?inline';
...
import mariadb from '@/assets/images/tools/mariadb.svg?inline';
...
import docker from '@/assets/images/tools/docker.svg?inline';
// import angular from '@/assets/images/tools/docker.svg?inline';
// import java from '@/assets/images/tools/java.svg?inline';
  ...
  data: function () {
    return {
      tools: [
        ...
        { key: 'flask',        icon: flask,        title: 'flask',        level: 'mid' },
        ...
        { key: 'mariadb',      icon: mariadb,      title: 'mariadb',      level: 'high' },
        { key: 'postman',      icon: postman,      title: 'postman',      level: 'high' },
        { key: 'git',          icon: git,          title: 'git',          level: 'high' },
        { key: 'docker',       icon: docker,       title: 'docker',       level: 'mid'  },
        { key: 'jenkins',      icon: jenkins,      title: 'jenkins',      level: 'mid'  },
        ...
        // { key: 'java',   icon: java,   title: 'java',   level: 'low'  },
      ]
    }
  }
};
